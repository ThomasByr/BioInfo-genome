...
from ..core import Tree, Value, create_data_from_stuff
...
folder_img = Image.open(BytesIO(base64.b64decode(folder_icon)))  # 12x12 pixels
file_img = Image.open(BytesIO(base64.b64decode(file_icon)))  # 12x12 pixels
...
class GenomeGUI:
    ...
    def __build_layout(self):
        layout = [
            [
                sg.Text(
                    "GENOME - by @ThomasByr, @m7415, @JBrandstaedt and @Bas6700",
                    font=self.__font,
                    expand_x=True,
                )
            ],
    ...
    def run(self):
        ...
        self.__data_component = {
            0: {"kind": DIR, "path": "", "file": os.path.basename(self.__results_folder), "children": None},
        }
            ...
            if event in {"-TREE-RIGHT-CLICK-", "Toggle Selection"}:
                for selected in values["-TREE-"]:
                    ...
                    if name in self.__selected_organisms:
                        ...
                        self.__component_lock.acquire()
                        self.__tree_component.update(
                            key=selected, icon=folder_icon if os.path.isdir(full_path) else file_icon
                        )
                        self.__component_lock.release()
                    else:
                        ...
                        self.__tree_component.update(
                            key=selected,
                            icon=folder_checked_icon if os.path.isdir(full_path) else file_checked_icon,
                        )
                        self.__component_lock.release()
                    ...
                    if node["children"] is not None:
                        ...
                        for item in files:
                            if item.is_file() and str(item) not in self.__known_files:
                                ...
                                icon = (
                                    folder_checked_icon
                                    if kind == DIR and item.name in self.__selected_organisms
                                    else folder_icon
                                    if kind == DIR
                                    else file_checked_icon
                                    if item.name in self.__selected_organisms
                                    else file_icon
                                )
                                self.__tree_data.insert(parent_key, key, str(file), [], icon=icon)
                                node["children"].append(key)
                                self.__data_component[key] = {
                                    "kind": kind,
                                    "path": path,
                                    "file": file,
                                    "children": None,
