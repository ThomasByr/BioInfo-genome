...
def create_data_from_stuff(name: str, path: str, NC_list: list[str], region: list[str]) -> int:
    ...
    for NC in NC_list:
        ...
        try:
            ...
            filename_suffix = "_" + record.annotations.get(
            ...
            for k in range(0, len(record.features)):
                if record.features[k].type in region:
                    ...
                    header = (
                        record.features[k].type
                        + " "
                        + record.annotations.get("organism", record.description.split(",")[0])
                        + " "
                        + record.name
                        + ":"
                    )
                    ...
                    if header2 != last_header:
                        ...
                        for i in range(len(bounds) - 1, -1, -2):
                            ...
                            header2 = header2.replace(
                                " " + bounds_tmp[i - 1] + ".", " " + str(bounds_inf) + ".", 1
                            )
                            header2 = header2.replace(
                                "." + bounds_tmp[i] + " ", "." + str(bounds_sup) + " ", 1
                            )
                                    ...
                                    for i in range(0, len(bounds) - 1):
                                        if i % 2 == 0:
                                            ...
                                            if complement:
                                                files[region.index(record.features[k].type)].write(
                                                    str(
                                                        record.seq[bounds_inf:bounds_sup].reverse_complement()
                                                    )
                                                    + "\n"
                                                )
                                            else:
                                                files[region.index(record.features[k].type)].write(
                                                    str(record.seq[bounds_inf:bounds_sup]) + "\n"
                                                )
